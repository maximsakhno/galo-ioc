...
from ioc.context import get_factory, add_factory, add_factory_decorator
...
def load() -> None:
    ...
    class FastAPIRoleChecker(RoleCheckerImpl):
        async def __call__(self, request: Request, user: User = Depends(current_user_resolver)) -> User:
